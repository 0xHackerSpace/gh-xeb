...
`gh xeb harness install <agent>` writes a `SKILL.md` teaching Claude Code or
Copilot how to drive the extension ([ADR-0022](adr/0022-harness-install.md)).

The command list in it is **generated by walking the live cobra tree**, and a
test walks the same tree and fails if the generator misses a command — so a new
subcommand is documented for agents automatically. The rules beside it are a
string constant and are *not* guarded that way: changing a command's safety
behaviour without editing `renderSkill` leaves the skill lying.

The four install paths were read off this machine, where both agents were
already installed with a `tfctl` skill. `.github/skills/` for Copilot project
scope is the one that was inferred rather than observed.
