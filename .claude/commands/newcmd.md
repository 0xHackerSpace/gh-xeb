...
1. `internal/cmd/$1.go` with a `new<Name>Cmd(deps Deps) *cobra.Command`
   constructor — `Short`, `Args`, and `RunE` set, output through
   `cmd.OutOrStdout()`.
2. Register it in `NewRootCmd` in `internal/cmd/root.go`.
3. If it needs GitHub data, go through `internal/gh` — add a narrow interface
   ...
   types.
4. `internal/cmd/$1_test.go` built from `testDeps()` and `runRoot` in
